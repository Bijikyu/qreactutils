...
function formatAxiosError(err) { //(normalize axios error)
  try {
    if (axios.isAxiosError(err)) { // normalize only axios errors
      const status = err.response?.status ?? 500; // fall back to 500 when status missing
      const data = err.response?.data ?? err.message; // prefer response data, fallback to message
      const dataString = typeof data === 'string' ? data : safeStringify(data); // ensure serializable for logs // use module for circular refs
      const error = new Error(`${status}: ${dataString}`); // preserve status and payload for caller
      return error;
    }
    ...
    return new Error(`Error formatting axios error: ${error.message}`); // failsafe if formatting throws
  }
}
