...
const axiosClient = axios.create({
  baseURL: typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000', // dynamic host for dev/prod parity
  withCredentials: true, // include session cookies in every request for auth
  headers: { "Content-Type": "application/json" } // default JSON payload header
}); // single axios instance to keep config centralized
...
async function codexRequest(requestFn, mockResponse) { //(handle codex offline logic)
  ...
  try {
    ...
    const res = await requestFn(); // perform real network request
...
async function apiRequest(url, method = 'POST', data) { //(public axios wrapper)
  try {
    ...
    return result; // forward pure data
...
const queryClient = new QueryClient({ // shared React Query client for the app
  defaultOptions: {
    queries: {
      queryFn: getQueryFn({ on401: "throw" }), // default query function handles auth errors
