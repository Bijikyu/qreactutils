...
const { showToast } = require('./utils'); // reuse centralized toast logic
 ...
 */
async function executeWithErrorToast(operation, toast, errorTitle = 'Error') { // wraps async call so failures trigger toast
  try {
 ...
 * @returns {*} Operation result or throws error
 */
async function executeWithToastFeedback(operation, toast, successMessage, errorTitle = 'Error') { // success and failure toasts wrapper
  try {
