...
// ID generation using counter for predictable, sequential IDs
let idCounter = 0

/**
 * Generate unique toast ID
 * Uses a simple counter approach for predictable, sequential IDs
 * with safe overflow handling.
 * 
 ...
 * Separated notification logic for cleaner code organization
...
const useAdvancedToast = useToast
 ...
 * Provides low-level access to state changes for custom integrations.
 * Returns an unsubscribe function for cleanup.
 * 
 * @param {Function} listener - Function called when state changes
 * @returns {Function} Unsubscribe function
 * 
 * @example
 * const unsubscribe = subscribe((state) => {
 *   console.log('Toast count:', state.toasts.length);
 * });
 * 
 * // Later, clean up
 * unsubscribe();
...
/**
 * Dismiss toast notification(s)
 * Handles the logic for dismissing single or multiple toasts.
 * Queues toasts for removal and dispatches the dismiss action.
 * 
 * @param {string} [toastId] - ID of toast to dismiss. If not provided, dismisses all toasts
 * 
 * @example
...
function dismissToast(toastId) {
  if (toastId) {
    ...
    getState().toasts.forEach((t) => addToRemoveQueue(t.id))
  }
 ...
 * Provides direct access to the current state for custom integrations
 * or testing scenarios where the hook approach isn't suitable.
 * 
 * @returns {Object} Current toast state
 * @returns {Array} returns.toasts - Array of current toast objects
 * 
 * @example
 * const currentState = getState();
 * console.log('Active toasts:', currentState.toasts.length);
 */
function getState() {
  return memoryState;
}
...
module.exports = {
  useToast,              // main hook for toast management // exported for component integration
  useAdvancedToast,      // alias for backward compatibility // exported for legacy support
  toast: advancedToast,  // imperative toast creation function // exported for programmatic toasts
  advancedToast,         // imperative toast creation function // exported for programmatic toasts (legacy)
  dismissToast,          // dismiss toast function // exported for programmatic dismissal
  ...
  subscribe,             // subscribe to state changes // exported for custom integrations
  getState,              // get current state // exported for direct state access
