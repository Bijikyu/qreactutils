 ...
 * 
 * Provides comprehensive copy-to-clipboard functionality with customizable feedback
 * and robust browser compatibility. This module addresses the complexity of clipboard
 * operations across different environments and browsers.
 * 
...
/**
 * Create a copy-to-clipboard function with customizable feedback
 * 
 * Factory pattern allows customization of feedback behavior per use case,
 * enabling different success/error messages for different contexts while
 * providing a consistent API across environments.
 * 
...
/**
 * Simple copy-to-clipboard utility without callbacks
 * 
 * Provides a straightforward interface for basic copy operations
 * without the need for success/error callbacks. Returns a promise
 * that resolves to boolean indicating success/failure.
 * 
 * @param {string} text - Text to copy to clipboard
 * @returns {Promise<boolean>} Promise resolving to true if successful, false otherwise
 */
async function copyToClipboard(text) {
  console.log('copyToClipboard simple version called');
  
  let success = false;
  let error = null;
  
  const copyFn = makeCopyFn(
    (copiedText) => { success = true; },
    (errorMsg) => { error = errorMsg; }
  );
  
  const result = await copyFn(text);
  console.log(`copyToClipboard simple version result: ${result}`);
