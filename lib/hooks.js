...
async function executeWithLoadingState(setIsLoading, asyncOperation) {
  try {
    ...
    return await asyncOperation(); // run caller provided operation
  } finally {
...
function useDropdownData(fetcher, toastFn, user) {
  ...
  const { data, isPending } = useQuery({ // query remote data via React Query
    queryKey, // unique cache key based on fetcher to reuse results
    queryFn: fetcher, // delegate actual request to provided fetcher
    enabled: !!user, // only run query when user is authenticated
    retry: false, // disable automatic retries to avoid unexpected calls
    onError: () => { // surface failure via toast when supplied
      if (typeof toastFn === 'function') { toastError(toastFn, `Failed to load data.`); }
    }
  }, queryClient);

  const fetchData = useCallback(
    () => queryClient.fetchQuery({ queryKey, queryFn: fetcher }), // force fetch bypassing cache
    [queryKey, fetcher]
  ); // memoized so identity stable across renders
...
function useEditForm(initialState) {
  ...
  return { editingId, fields, setField, startEdit, cancelEdit }; // expose editing controls to caller
}
 ...
 */
const TOAST_LIMIT = 1; // only show one toast at a time to reduce clutter
 ...
 */
const TOAST_REMOVE_DELAY = 1000000; // delay before toast cleanup to allow long animations
...
const addToRemoveQueue = (toastId) => { // schedule toast removal after delay
  ...
  const timeout = setTimeout(() => {
    toastTimeouts.delete(toastId); // cleanup reference once timeout fires
    dispatch({ // instruct reducer to drop toast from state
      type: "REMOVE_TOAST", // action signals permanent removal
      toastId: toastId, // specific toast id to remove
    });
  }, TOAST_REMOVE_DELAY);
...
const reducer = (state, action) => { // state machine controlling toast lifecycle
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      if (toastId) {
        addToRemoveQueue(toastId); // start timeout for specific toast
      } else {
        state.toasts.forEach((toast) => {
          addToRemoveQueue(toast.id); // dismiss all toasts when id missing
        });
      }
