...
const { createSubTrigger, createContextMenuSubTrigger, createMenubarSubTrigger } = require('./components'); // import component factories
...
const { useAdvancedToast, advancedToast, getAdvancedToastCount, clearAllAdvancedToasts, getAdvancedToastTimeoutCount, reducer: advancedToastReducer, actionTypes: advancedToastActionTypes, dispatch: advancedToastDispatch } = require('./advancedToast'); // import advanced toast system
...
/**
 * React hook for detecting mobile viewport sizes using react-responsive.
 *
...
/**
 * Alternative mobile detection hook using usehooks-ts library
 *
 * This provides a simpler, more direct approach to media queries using the
 * usehooks-ts library. Uses a standard CSS media query string for mobile
 * detection at 767px breakpoint, which is commonly used across many frameworks.
 *
 * @returns {boolean} True if the viewport is 767px or narrower (mobile)
 */
function useIsMobileTS() {
  console.log(`useIsMobileTS is running`); // entry log for debugging
  const result = useMediaQueryTS("(max-width:767px)"); // detect mobile using standard breakpoint
  console.log(`useIsMobileTS is returning ${result}`); // exit log for tracing
  return result; // boolean indicating if viewport is mobile size
}
...
module.exports = { // consolidated export for entire hooks library
  ...
  useIsMobileTS,       // alternative mobile detection using usehooks-ts // exported for simpler media query approach
