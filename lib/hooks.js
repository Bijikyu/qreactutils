...
const { useToast, useAdvancedToast, toast, advancedToast, getAdvancedToastCount, clearAllAdvancedToasts, getAdvancedToastTimeoutCount, clearToastTimeout, subscribe: advancedToastSubscribe, getState: advancedToastGetState, reducer: advancedToastReducer, actionTypes: advancedToastActionTypes, dispatch: advancedToastDispatch } = require('./advancedToast'); // import advanced toast system
...
// Removed react-responsive dependency - now using native window.matchMedia in useIsMobile
 ...
 *
 * Uses native window.matchMedia for better performance and fewer dependencies.
...
function useIsMobile() {
  ...
  return result // boolean indicating viewport size based on window.innerWidth
}
...
/**
 * React hook for clipboard operations with toast feedback
 * 
 * Provides a simple interface for copying text to clipboard with integrated
 * toast notifications for success/error feedback. Combines the robust clipboard
 * functionality with the library's toast system for consistent user experience.
 * 
 * Key features:
 ...
 * @param {string} options.successMessage - Custom success message (defaults to "Copied to clipboard")
 * @param {string} options.errorMessage - Custom error message prefix (defaults to "Failed to copy")
 * @returns {Array} Returns [copyText, isLoading] tuple
 */
function useClipboard(options = {}) {
  console.log('useClipboard is running with options:', safeStringify(options));
  ...
  const { toast } = useAdvancedToast(); // get toast system for feedback
  const [isLoading, setIsLoading] = useState(false);
  
  // Create stable copy function with toast integration
  const copyText = useCallback(async (text) => {
    console.log(`useClipboard copyText called with text length: ${text ? text.length : 0}`);
    logger.debug('useClipboard: copy operation initiated', {
    ...
    return executeWithLoadingState(setIsLoading, async () => {
      const copyFn = makeCopyFn(
        (copiedText) => {
          // Success callback - show success toast
          toast({
            title: "Success",
            description: successMessage,
            variant: "success"
          });
          console.log('useClipboard: copy successful, toast shown');
        },
        (errorMsg) => {
          ...
          console.log('useClipboard: copy failed, error toast shown');
        }
      );
      
      const result = await copyFn(text);
      console.log(`useClipboard copyText result: ${result}`);
      return result;
    });
  }, [toast, successMessage, errorMessage]);
  
  console.log('useClipboard is returning copyText function and loading state:', isLoading);
  return [copyText, isLoading];
}

/**
 * React hook for clipboard operations with custom callbacks
 * 
 * Provides more control over clipboard operations by allowing custom success
 * and error handlers. Useful when you need specific behavior on copy success/failure
 * beyond just showing toast notifications.
 * 
 * @param {Object} callbacks - Success and error callback functions
 * @param {Function} callbacks.onSuccess - Called when copy succeeds (receives copied text)
 * @param {Function} callbacks.onError - Called when copy fails (receives error message)
 * @returns {Array} Returns [copyText, isLoading] tuple
 */
function useClipboardWithCallbacks(callbacks = {}) {
  console.log('useClipboardWithCallbacks is running with callbacks');
  
  const { onSuccess, onError } = callbacks;
  const [isLoading, setIsLoading] = useState(false);
  
  // Create stable copy function with custom callbacks
  const copyText = useCallback(async (text) => {
    console.log(`useClipboardWithCallbacks copyText called with text length: ${text ? text.length : 0}`);
    
    return executeWithLoadingState(setIsLoading, async () => {
      const copyFn = makeCopyFn(onSuccess, onError);
      const result = await copyFn(text);
      console.log(`useClipboardWithCallbacks copyText result: ${result}`);
      return result;
    });
  }, [onSuccess, onError]);
  
  console.log('useClipboardWithCallbacks is returning copyText function and loading state:', isLoading);
  return [copyText, isLoading];
}

/**
 * Storage keys for API key preferences
 */
const STORAGE_KEY = 'example_api_key';
const STORAGE_USE = 'api_key_use_examples';

/**
 * Read example API key from environment in a way that works under Vite and Jest.
 * Avoid direct `import.meta` to keep Jest (CJS) happy.
 */
const ENV_DEFAULT = (() => {
  try {
    // Attempt to read via dynamic eval to avoid parse-time syntax errors in Jest
    // eslint-disable-next-line no-eval
    const meta = eval('import.meta');
    return meta?.env?.VITE_EXAMPLE_API_KEY;
  } catch {
    try { 
      return process?.env?.VITE_EXAMPLE_API_KEY; 
    } catch { 
      return undefined; 
    }
  }
...
/**
 * React hook for managing API key preferences with localStorage persistence
 *
 * Provides state management for API keys with automatic persistence to localStorage
 * and fallback to environment variables. Includes a toggle for whether to use
 * the API key in examples.
 *
 * @returns {Object} API key management state and functions
 * @returns {string} returns.apiKey - Current API key value
 * @returns {Function} returns.setApiKey - Function to save new API key
 * @returns {Function} returns.clearApiKey - Function to clear API key
 * @returns {boolean} returns.useInExamples - Whether to use API key in examples
 * @returns {Function} returns.setUseInExamples - Function to toggle example usage
 */
function useApiKeyPref() {
  console.log('useApiKeyPref is running'); // entry log for debugging
  
  const [apiKey, setApiKey] = useState('');
  const [useInExamples, setUseInExamples] = useState(false);
  ...
  useEffect(() => {
    try {
      ...
      if (saved != null) {
        ...
        setApiKey(ENV_DEFAULT);
      }
    } catch (error) {
      console.log('useApiKeyPref failed to load API key from localStorage:', error);
    }
    
    try {
      const flag = localStorage.getItem(STORAGE_USE);
      if (flag != null) {
        setUseInExamples(flag === 'true');
      }
    } catch (error) {
      console.log('useApiKeyPref failed to load usage flag from localStorage:', error);
    }
  }, []);

  const save = useCallback((next) => {
    console.log('useApiKeyPref save called with key length:', next ? next.length : 0);
    setApiKey(next);
    try { 
      localStorage.setItem(STORAGE_KEY, next); 
    } catch (error) {
      console.log('useApiKeyPref failed to save API key to localStorage:', error);
    }
  }, []);

  const clear = useCallback(() => {
    console.log('useApiKeyPref clear called');
    setApiKey('');
    try { 
      localStorage.removeItem(STORAGE_KEY); 
    } catch (error) {
      console.log('useApiKeyPref failed to clear API key from localStorage:', error);
    }
  }, []);

  const toggle = useCallback((next) => {
    console.log('useApiKeyPref toggle called with value:', next);
    setUseInExamples(next);
    try { 
      localStorage.setItem(STORAGE_USE, String(next)); 
    } catch (error) {
      console.log('useApiKeyPref failed to save usage flag to localStorage:', error);
    }
  }, []);
  ...
  const result = { 
    ...
    clearApiKey: clear, 
    useInExamples, 
    setUseInExamples: toggle 
  };
  
  console.log('useApiKeyPref is returning state with apiKey length:', apiKey.length, 'useInExamples:', useInExamples);
  return result;
}
...
const { makeCopyFn, copyToClipboard } = require('./clipboard'); // clipboard utilities
...
module.exports = { // consolidated export for entire hooks library
  ...
  useIsMobile,         // viewport size detection using native window.matchMedia // public for uniform responsive checks
  ...
  useToast,            // main toast hook with clean modular pattern // exported for modern toast management
  useAdvancedToast,    // advanced toast notification system hook // exported for comprehensive toast management
  toast,               // imperative toast creation function // exported for programmatic toast notifications
  advancedToast,       // imperative advanced toast creation // exported for programmatic toast notifications (legacy)
  ...
  executeWithErrorToast, // async operation with error toast // exported for error handling with toasts
  executeWithToastFeedback, // async operation with toast feedback // exported to reduce boilerplate
  ...
  // Clipboard utilities
  useClipboard,             // clipboard hook with toast feedback // exported for copy-to-clipboard with notifications
  useClipboardWithCallbacks, // clipboard hook with custom callbacks // exported for advanced clipboard handling
  useApiKeyPref,            // API key preference management with localStorage persistence // exported for API key state management
  makeCopyFn,               // clipboard function factory // exported for custom clipboard implementations
  copyToClipboard           // simple clipboard utility // exported for basic copy operations
};